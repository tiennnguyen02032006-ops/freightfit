...
export function CargoDetailPopup({ container }: CargoDetailPopupProps) {
  ...
  // Thu gọn lại hướng dẫn mỗi khi đổi sang kiện hàng khác — tránh giữ "đã mở hint" của kiện trước
  // sang kiện sau, dễ gây cảm giác bảng tự dưng cao hơn bình thường mà không rõ vì sao.
  useEffect(() => {
    setShowHelp(false);
  }, [selectedPlacementId]);
  ...
  return (
    <div className="cargo-detail-popup">
      ...
      {cannotRotate && <p className="cargo-detail-popup-hint">Không xoay được — loại hàng này ở chế độ "Không xoay"</p>}
      {showHelp && (
